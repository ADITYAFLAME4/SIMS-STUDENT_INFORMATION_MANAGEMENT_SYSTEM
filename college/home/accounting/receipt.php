...
$fees = $conn->query("SELECT ef.*,s.firstName as sname,s.lastName,s.admissionNumber,concat(c.course,' - ',c.level) as `class` FROM student_ef_list ef inner join tblstudents s on s.id = ef.student_id inner join courses c on c.id = ef.course_id  where ef.id = {$_GET['ef_id']}");
...
<div class="container-fluid">
	<p class="text-center" style="color:red; font-size:30px;"><b><?php echo $_GET['pid'] == 0 ? "TECHNO ENGINEERING COLLEGE BANIPUR" : 'Payment Receipt' ?></b></p>
	<hr>
	<p class="text-center" style="color:red;"><b><?php echo $_GET['pid'] == 0 ? "Banipur,Ashoknagar,24 Pargans North ,West Bengal,India" :'' ?></b></p>
	<hr>
	<p class="text-center" style="color:red;"><b><?php echo $_GET['pid'] == 0 ? "Phone :03216238138" : '' ?></b></p>
	<hr>
	<div class="flex">
		<div class="w-50">
			<p>Bill No: <b><?php echo $ef_no ?></b></p>
			<p>Student: <b><?php echo ucwords($sname) ?> <?php echo ucwords($lastName) ?></b></p>
			<p>Course/Level: <b><?php echo $class ?></b></p>
			<p>ID NO: <b><?php echo $admissionNumber ?></b></p>
		</div>
		<?php if($_GET['pid'] > 0): ?>
		<div class="w-50">
	...
	<table class="wborder">
		<tr>
			...
			<p><b>Payment Details</b></p>
				<table width="100%" class="wborder" color="red">
					<tr>
						<td width="50%">Date</td>
						<td width="50%" class='text-right'>Amount</td>
					</tr>
